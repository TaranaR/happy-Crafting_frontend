...
import { CREATE_TYPE_OF_PRODUCT_RESET } from "../../constants/adminConstants";
...
export default function ManageTypeOfProduct() {
  ...
  const getSubCategoryInfo = useSelector((state) => state.sellerGetSubCategory);
  const getAllTypeOfProductInfo = useSelector(
    (state) => state.adminGetAllTypeOfProduct
  );
  const createTypeOfProductInfo = useSelector(
    (state) => state.adminCreateTypeOfProduct
  );

  const { mainCatInfo } = getMainCategoryInfo;
  const { subCatInfo } = getSubCategoryInfo;
  const { typeInfo } = getAllTypeOfProductInfo;
  const { typeOfProductInfo, loading, error } = createTypeOfProductInfo;
  let main = [];
  let sub = [];

  useEffect(() => {
    dispatch(getAllTypeOfProduct());
    if (!typeInfo) {
      dispatch(getAllTypeOfProduct());
    } else {
      //setData(subCatInfo);
  ...
  const handleOpen = () => {
    dispatch({ type: CREATE_TYPE_OF_PRODUCT_RESET });
    setOpen(true);
    dispatch(getMainCategory());
  };
  const handleClose = () => setOpen(false);

  const deleteTypeOfProductHandler = () => {
    dispatch(deleteTypeOfProduct(typeId));
    dispatch(getAllTypeOfProduct());
  };

  const submitHandler = () => {
    if (subCatId === "" || mainCatId === "" || typeName === "") return;
    ...
    dispatch(createTypeOfProduct(type));
          ...
          <Box className={classes.modelWrapper}>
            <Container className={classes.formWrapper}>
              ...
              <Grid item xs={12}>
                ...
                {typeOfProductInfo && setOpen(false)}
                {error && error}
              </Grid>
            </Container>
          </Box>
        ...
        <Grid container>
          ...
          <Grid item xs={12} style={{ textAlign: "right", marginTop: "20px" }}>
            ...
            <Button
              ...
              onClick={deleteTypeOfProductHandler}
            >
              Delete
            </Button>
          </Grid>
